...
void cache_sim_t::init()
{
  ...
  time = 0;
  timeTable = new uint64_t[sets * ways]();
  std::fill_n(timeTable, sets * ways, 0);
  countTable = new uint64_t[sets * ways]();
  std::fill_n(countTable, sets * ways, 0);
}
...
uint64_t *cache_sim_t::check_tag(uint64_t addr)
{
  ...
  for (size_t i = 0; i < ways; i++)
    ...
    {
      // hit
      timeTable[idx * ways + i] = 0;
      countTable[idx * ways + i]++;
...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  ...
  // deciding victim index
  uint64_t victim_index;
  uint64_t victim_time = 0;
  uint64_t victim_count = std::numeric_limits<uint64_t>::max();
  for (size_t i = 0; i < ways; i++)
  {
    if (countTable[idx * ways + i] <= victim_count)
    {
      if (victim_time <= timeTable[idx * ways + i])
      {
        victim_index = i;
        victim_time = timeTable[idx * ways + i];
        victim_count = countTable[idx * ways + i];
      }
    }
  }

  // for finding victim
  uint64_t victim = tags[idx * ways + victim_index];

  // for storing addr
  tags[idx * ways + victim_index] = (addr >> idx_shift) | VALID;

  // time & table
  timeTable[idx * ways + victim_index] = 0;
  countTable[idx * ways + victim_index] = 0;
